...
import { useCallStore, type OutgoingCall } from '@/store';
...
export const OutgoingCallModal: React.FC = () => {
    ...
    return (
        <div className="fixed inset-0 z-[var(--z-call)] flex items-center justify-center font-light tracking-tight">
            ...
            {/* Content */}
            <div className="relative z-10 flex flex-col items-center text-center px-8 w-full max-w-md">
                {/* User Info / Avatar Section */}
                <div className="relative mb-10 flex flex-col items-center">
                    <div className="relative mb-6">
                        <div className={`absolute inset-0 rounded-full blur-3xl opacity-30 ${outgoingCall.status === 'rejected'
                            ? 'bg-[var(--danger)]'
                            : 'bg-[var(--accent-primary)] animate-pulse'
                            }`} />
                        <UserAvatar
                            name={outgoingCall.targetName}
                            avatar={outgoingCall.targetAvatar}
                            size="xl"
                            className="relative z-10 border-4 border-[var(--glass-border)] shadow-glow"
                        />

                        {/* Call type icon badge */}
                        <div className="absolute -bottom-2 -right-2 z-20 w-10 h-10 rounded-full bg-[var(--background-secondary)] flex items-center justify-center border border-[var(--glass-border)] shadow-lg">
                    ...
                    <h2 className="text-3xl font-light tracking-tight text-white mb-2 truncate max-w-xs">
                        {outgoingCall.targetName || (outgoingCall.isVideoCall ? 'Video Call' : 'Voice Call')}
                    </h2>
                    <p className={`text-lg font-light tracking-tight ${outgoingCall.status === 'rejected' ? 'text-[var(--danger)]' : 'text-[var(--text-muted)]'
                        }`}>
                        {getStatusMessage()}
                    </p>
                </div>
                ...
                {outgoingCall.status === 'ringing' && (
                    <div className="flex gap-1.5 mb-12">
                        <div className="w-2.5 h-2.5 rounded-full bg-[var(--accent-primary)] animate-bounce" style={{ animationDelay: '0ms' }} />
                        <div className="w-2.5 h-2.5 rounded-full bg-[var(--accent-primary)] animate-bounce" style={{ animationDelay: '150ms' }} />
                        <div className="w-2.5 h-2.5 rounded-full bg-[var(--accent-primary)] animate-bounce" style={{ animationDelay: '300ms' }} />
                    </div>
                )}
                ...
                {outgoingCall.status === 'ringing' && (
                    ...
                    >
                        <div className="w-16 h-16 rounded-full bg-[var(--danger)] text-white flex items-center justify-center transition-all duration-300 group-hover:scale-110 group-hover:shadow-lg group-hover:shadow-[var(--danger)]/40 active:scale-95">
